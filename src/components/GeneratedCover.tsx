...
// Performance-safe automation: only one PDF cover render at a time.
...
function openCoverDb() {
  ...
  coverDbPromise = new Promise((resolve, reject) => {
    const request = indexedDB.open('maktabah-pdf-covers', 1);
...
function canRenderPdfCover() {
  ...
  // To protect phones and weak devices from heating, use generated covers there.
...
export default function GeneratedCover({ book, height = '100%', width = '100%', radius = 0, fontSize = 14, compact = false }: Props) {
  ...
  useEffect(() => {
    return () => {
      if (cachedCoverUrl) URL.revokeObjectURL(cachedCoverUrl);
    };
  }, [cachedCoverUrl]);
    ...
    async function renderFirstPage() {
      try {
        ...
        if (!cancelled) {
          setPdfRendered(true);
          canvas.toBlob(blob => {
            if (blob) setCachedCover(cacheKey, blob);
          }, 'image/webp', 0.82);
        }
      } catch {
